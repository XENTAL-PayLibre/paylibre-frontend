...
export const COOKIE_KEYS = {
  access_token: 'plb_access',
  refresh_token: 'plb_refresh',
...
export function getRefreshToken(): string | null {
  ...
  const hasSentinel =
    ...
    (typeof localStorage !== 'undefined' &&
      localStorage.getItem(COOKIE_KEYS.has_refresh));
 ...
 * The real session (pl_access/pl_refresh) is HttpOnly and lives on the API's domain, so it is
 ...
 */
export const SESSION_SENTINEL = 'plb_session';

export function markSession(days = 14) {
  if (typeof window === 'undefined') return;
  cookies.set(SESSION_SENTINEL, '1', {
    path: '/',
    sameSite: 'lax',
